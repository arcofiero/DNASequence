...
public class CodingDNASequence extends DNASequence
{
  ...
  */
  public boolean checkStartCodon(){
	  String se= this.toString();
    if(seqLength()>=3) { // when the length of seqLength is less than 3
    	if(se.substring(0,3).equalsIgnoreCase("atg")) { // if first 3 characters of a string is equal to {a,t,g} regardless of uppercase or lowercase
   ...
   * The translation stops if the method {@link #getAminoAcid(String)} returns the character {@code '$'}, 
   ...
   * The method returns the protein sequence in a new character array,
    	where the length of the protein sequence is equal to the length of the array.
   * Note that the first codon is a string consisting of the three characters in {@code seqarr[0]}, 
   	 	{@code seqarr[1]}, and {@code seqarr[2]}; the second codon is made of {@code seqarr[3]}, {@code seqarr[4]}, 
   	 	and {@code seqarr[5]}; and so on. The last codon may contain one or two characters if the length of the array {@code seqarr} 
  ...
  */
  public char[] translate()throws RuntimeException{
    	String temp = "";
    	if(checkStartCodon() == false) {		//when there's no starting codon, this line throws a new RuntimeException("No start codon")
    		...
    		while( i< seqLength()){ 	
    			...
    			if(getAminoAcid(toString().substring(i, j)) =='$'){ 
    				break;		// When getAminoAcid from starting point i to j equals '$' then the search stops
    			}
    			else{
    				temp = temp + getAminoAcid(toString().substring(i, j));  
    			}
    			i = i + 3;
    			j = j + 3; //For reading the sequencing whether while loop returns true or false.
    		}//while loop ends
    	}
